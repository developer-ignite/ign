 ...
 *
 * Override of: [the-events-calendar]/src/views/modules/meta.php
 *
 * Only change from the plugin default: the Venue and Organizer sections
 * are also shown when the event has no linked Venue/Organizer post but
 * does have a plain-text name captured at ICS import (see
 * inc/helpers/myignite-image-sync.php and the matching overrides of
 * modules/meta/venue.php and modules/meta/organizer.php in this folder).
 *
 * @version 4.6.10
 *
...
$has_venue     = tribe_get_venue_id() || get_post_meta( get_the_ID(), '_myignite_venue_name', true );
$has_organizer = tribe_has_organizer() || get_post_meta( get_the_ID(), '_myignite_organizer_names', true );
?>
...
if ( $has_venue ) {
	...
	} elseif ( ! $set_venue_apart && ! $has_organizer && tribe_embed_google_map() ) {
